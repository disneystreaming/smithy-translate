...
private[compiler] abstract class RefParser(ns: Path) {
      ...
      .flatMap { uri =>
        Option(uri.getScheme()) match {
          ...
          case _ =>
            Left(
              ToSmithyError
                .BadRef(s"Unsupported URI scheme: ${uri.getScheme()}")
            )
        }
      }
  }
  ...
  private def parseURI(
    ...
    def parseWithPath(
      ...
      val namespaceParts = baseNamespace.initLast
        .map(_._1.toList)
        .getOrElse(Nil) ++ pathSegsIn :+ fileName
  ...
  def parseRemoteURI(uri: URI): Either[ToSmithyError, DefId] = {
    ...
    parseURI(Chain.empty, uri).map(defId =>
      defId.copy(namespace =
        Namespace(baseNamespace.toList ++ defId.namespace.segments)
      )
    )
  }
  ...
  private def deriveNameFromFragment(
  ...
  private def normalizeRelativePath(
      uri: URI,
      parts: List[String]
  ): Either[ToSmithyError, Chain[String]] = {
    parts.foldLeft(Chain.empty[String].asRight[ToSmithyError]) {
      ...
      case (Right(acc), "..") =>
        acc.initLast match {
          ...
          case None            =>
            Left(ToSmithyError.Restriction(s"Ref $uri goes too far up"))
        }
