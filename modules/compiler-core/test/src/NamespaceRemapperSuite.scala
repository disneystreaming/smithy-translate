...
final class NamespaceRemapperSuite extends munit.FunSuite {
  ...
  test(
    "remap namespace list should remap the namespace starting from the left"
  ) {
  ...
  test(
    "remap namespace list should strip the prefix when there is an empty target namespace"
  ) {
    val remapper = new NamespaceRemapper(
      Map(
  ...
  test("remap DefId should remap the namespace of the DefId") {
    ...
    assertEquals(
      remapper.remap(defId),
      DefId(Namespace(List("c", "d")), Name.arbitrary("MyShape"))
    )
  }
